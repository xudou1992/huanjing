...
function x300022_DiscoverCorpseMonster(sceneId, selfId)
	...
	-- 定义马贼角色类型和对应的属性倍率
	local banditRoles = {
		{name="马贼首领", multiplier=5.0},    -- 首领：最强战力
		{name="马贼副手", multiplier=4.0},    -- 副手：次强战力  
		{name="精英马贼", multiplier=3.5},    -- 精英1
		{name="精英马贼", multiplier=3.5},    -- 精英2
		{name="老练马贼", multiplier=3.0},    -- 老手1
		{name="老练马贼", multiplier=3.0},    -- 老手2
		{name="老练马贼", multiplier=3.0},    -- 老手3
		{name="新手马贼", multiplier=2.5},    -- 新手1
		{name="新手马贼", multiplier=2.5},    -- 新手2
		{name="新手马贼", multiplier=2.5}     -- 新手3
	}
	...
	for i=1, 10 do
		...
		if i == 1 then
			-- 首领：玩家正前方3格（最显眼位置）
			x, z = playerX + 3, playerZ
		elseif i == 2 then
			-- 副手：玩家左前方3格
			x, z = playerX + 3, playerZ - 3
		elseif i == 3 then
			-- 精英1：玩家右前方3格
			x, z = playerX + 3, playerZ + 3
		elseif i == 4 then
		...
		elseif i == 7 then
			-- 老手3：玩家正左方3格
			x, z = playerX, playerZ - 3
		elseif i == 8 then
			-- 新手1：玩家正右方3格
			x, z = playerX, playerZ + 3
		elseif i == 9 then
			-- 新手2：玩家左前方1格（内圈）
			x, z = playerX + 1, playerZ - 1
		else
			-- 新手3：玩家右前方1格（内圈）
			x, z = playerX + 1, playerZ + 1
		end
		...
		-- 根据马贼角色差异化提升属性
		local role = banditRoles[i]
		local multiplier = role.multiplier
		
		local baseMaxHP = LuaFnGetMaxBaseHp(sceneId, MonsterId)
		local baseAttackPhysics = LuaFnGetBaseAttackPhysics(sceneId, MonsterId)
		local baseDefencePhysics = LuaFnGetBaseDefencePhysics(sceneId, MonsterId)
		local baseDefenceMagic = LuaFnGetBaseDefenceMagic(sceneId, MonsterId)
		
		-- 计算要增加的属性值（基础值 * (倍率-1)，使总属性变为倍率倍）
		local hpIncrease = baseMaxHP * (multiplier - 1)
		local attackIncrease = baseAttackPhysics * (multiplier - 1)
		local defencePhysicsIncrease = baseDefencePhysics * (multiplier - 1)
		local defenceMagicIncrease = baseDefenceMagic * (multiplier - 1)
		
		-- 获取当前修正值并设置新的修正值
		local currentHpRefix = LuaFnGetLifeTimeAttrRefix_MaxHP(sceneId, MonsterId)
		local currentAttackRefix = LuaFnGetLifeTimeAttrRefix_AttackPhysics(sceneId, MonsterId)
		local currentDefencePhysicsRefix = LuaFnGetLifeTimeAttrRefix_DefencePhysics(sceneId, MonsterId)
		local currentDefenceMagicRefix = LuaFnGetLifeTimeAttrRefix_DefenceMagic(sceneId, MonsterId)
		
		-- 设置新的修正值
		LuaFnSetLifeTimeAttrRefix_MaxHP(sceneId, MonsterId, currentHpRefix + hpIncrease)
		LuaFnSetLifeTimeAttrRefix_AttackPhysics(sceneId, MonsterId, currentAttackRefix + attackIncrease)
		LuaFnSetLifeTimeAttrRefix_DefencePhysics(sceneId, MonsterId, currentDefencePhysicsRefix + defencePhysicsIncrease)
		LuaFnSetLifeTimeAttrRefix_DefenceMagic(sceneId, MonsterId, currentDefenceMagicRefix + defenceMagicIncrease)
	...
	BeginEvent(sceneId)
		AddText(sceneId, "马贼团伙出现！小心他们的围攻战术！");
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
	
	local _, sceneName = CallScriptFunction(x300022_g_ChengxiongdatuScriptId, "GetScenePosInfo", sceneId,sceneId)
	local playerName = GetName(sceneId,selfId)
	local strText = format("#W#{_INFOUSR%s}#P在挖宝时不慎泄漏消息，在#G%s#P引来了一个#{_BOSS48}。这伙马贼有首领统率，组织严密，江湖志士务必小心应对！剿灭他们可获得丰厚藏宝！", 
...
function x300022_OnDefaultEvent( sceneId, selfId, BagPos)
	...
	--PrintStr(sceneName)
	local strText = ScriptGlobal_Format("#{DTZDXL_210319_02}", sceneName, targetX, targetZ,targetSceneId)
	...
	elseif ret < 85 then --放出僵尸
		x300022_DiscoverCorpseMonster(sceneId, selfId)
	elseif ret < 10 then --获得物品
	...
	-- elseif ret < 70 then --遇盗墓贼
		-- x300022_DiscoverGhoulMonster(sceneId, selfId)
	-- elseif ret < 80 then --掉入宝藏
  ...
  EraseItem( sceneId, selfId, BagPos )
	...
	LuaFnAuditWaBao(sceneId, selfId)
	--周活跃数据埋点
	LuaFnFinishZhouHuoYueMission(sceneId,selfId,4)
end

function x300022_OnConditionCheck(sceneId,selfId)
	return 1;
end
