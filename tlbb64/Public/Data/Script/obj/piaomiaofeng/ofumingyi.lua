...
function x402273_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x402273_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "ShuangZi" ) then
			AddNumText( sceneId, x402273_g_ScriptId, "挑战不平道人和卓不凡", 10, 1 )
		end
...
function x402273_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗5秒钟后开始" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗4秒钟后开始" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗3秒钟后开始" )
		return
	end
	...
	if 4 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗2秒钟后开始" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗1秒钟后开始" )
		return
	end

	if 2 == step then
		--提示战斗开始....
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "战斗开始" )
		return
	end
