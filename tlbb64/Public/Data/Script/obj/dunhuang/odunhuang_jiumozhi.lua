...
function x008011_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent( sceneId )
		AddText(sceneId,"  施主风尘仆仆来到玉门关，是要去西夏国吗？恕小僧直言，施主印堂发暗，此去西夏国必然凶多吉少，还是及早返回中原，才是万全之策。")

		local missionIndex = GetScriptIDByMissionID( sceneId, selfId, x008011_g_RSMissionId )
		if missionIndex ~= -1 then
			local missionName = TGetMissionName( missionIndex )
			if missionName ~= "鸠摩智任务" and IsMissionHaveDone( sceneId, selfId, x008011_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x008011_g_scriptId, "鸠摩智任务", 3, 1 )
			end
...
function x008011_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- 鸠摩智任务
		if IsHaveMission( sceneId, selfId, x008011_g_RSMissionId ) > 0 then
			x008011_NotifyFailBox( sceneId, selfId, targetId, "    哦，你已有其他英雄的关系任务（剧情循环任务）尚未完成，请完成之后再来找我吧。" )
			return
		end
