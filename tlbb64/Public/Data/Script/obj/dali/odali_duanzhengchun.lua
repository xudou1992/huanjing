...
function x002004_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)	
		...
		if missionIndex ~= -1 then
			...
			if missionName ~= "段誉任务" and IsMissionHaveDone( sceneId, selfId, x002004_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x002004_g_scriptId, "段誉任务", 3, 1 )
			end
...
function x002004_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- 段誉任务
		if IsHaveMission( sceneId, selfId, x002004_g_RSMissionId ) > 0 then
			x002004_NotifyFailBox( sceneId, selfId, targetId, "    哦，你已有其他英雄的关系任务（剧情循环任务）尚未完成，请完成之后再来找我吧。" )
			return
		end
