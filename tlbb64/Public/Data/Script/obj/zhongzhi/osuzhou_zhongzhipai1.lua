...
x714093_g_eventList={20104001,20104002,20104005,20104008,20104009,20104010,
			20105001,20105003,20105006,20105009,20105010} --植物的编号,不是scriptId
--**********************************
--事件交互入口
--**********************************
function x714093_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714093_g_ScriptId, "种植早产植物",6,254)
			AddNumText(sceneId, x714093_g_ScriptId, "种植晚产植物",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
function x714093_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--通过x714093_g_eventList和scriptglobal中的植物列表对比，并根据玩家种植技能等级来显示相应植物
			for i, eventId in x714093_g_eventList do	--遍历这个稻草人可以种植的植物列表
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--遍历scriptglobal中的所有植物列表
					if eventId == g_ZhiWuId then
						...
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --如果玩家种植技能等级>=该植物要求技能等级]
							...
							AddNumText(sceneId, x714093_g_ScriptId, V_ZHONGZHI_NAME[name_Index].."("..V_ZHONGZHI_NEEDLEVEL[j].."级)",6,name_Index)
							break
						end
					end
				end
