...
function	 x712010_OnOpen(sceneId,selfId,targetId)
	...
	--获得随机数,设置一次钓到鱼的时间
	x712010_g_FishTime = random(80000)+20000	--获得一个20-100秒之间的随机数
	--x712010_g_FishTime = random(8000)+2000	--获得一个2-10秒之间的随机数
	SetAbilityOperaTime(sceneId,selfId,x712010_g_FishTime)	--设置一次钓到鱼的时间
...
function	x712010_OnProcOver(sceneId,selfId,targetId)
	...
	if ret_1 > 0 then					-->0表示物品成功放入背包中
		Msg2Player(sceneId,selfId,"你钓到一条鲑鱼。",MSG2PLAYER_PARA)
		-- 增加熟练度
		ABilityID	=	GetItemBoxRequireAbilityID(sceneId,targetId)
