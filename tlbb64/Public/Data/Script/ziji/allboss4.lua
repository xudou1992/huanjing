...
x893005_g_AllBoss = {19}
--##18是场景ID，参考SceneInfo.ini

--##这里是系统公告，根据分组ID发布公告，同一组BOSS只发布一次
x893005_g_BossSysMsgByGroupID={}
x893005_g_BossSysMsgByGroupID[19]={Msg="#cff99cc雁北的#Y中手BOSS#cff99cc已出现在#G雁北#W（170，54  167，135  241，155  68，177）",isSended=0}

x893005_g_AllBoss[19]=
{	
	{ ID=55042, GroupId=19, Title="", PosX=170,  PosY=54,  BaseAI=0, ExtAIScript=259, ScriptID=-1 },	
	{ ID=55042, GroupId=19, Title="", PosX=167,  PosY=135,  BaseAI=0, ExtAIScript=259, ScriptID=-1 },
	{ ID=55042, GroupId=19, Title="", PosX=241,  PosY=155,  BaseAI=0, ExtAIScript=259, ScriptID=-1 },
	{ ID=55042, GroupId=19, Title="", PosX=68,  PosY=177,  BaseAI=0, ExtAIScript=259, ScriptID=-1 },
}
...
--刷怪逻辑
--**********************************
function x893005_OnCharacterTimer( sceneId, objId, dataId, uTime )
	local nHour	 = GetHour()--小时
	local nMinute = GetMinute()--分钟
	
	if sceneId==19 then	--##玄海
	    if (nHour==2 and nMinute==00) 
		or (nHour==4 and nMinute==00) 
		or (nHour==6 and nMinute==16) 
...
function x893005_CreateMonster( sceneId )
	...
	for i,data in x893005_g_AllBoss[sceneId] do
		...
		AddGlobalCountNews( sceneId, x893005_g_BossSysMsgByGroupID[19].Msg )
end
