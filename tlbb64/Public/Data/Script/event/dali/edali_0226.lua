...
--任务入口函数
--**********************************
function x210226_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
...
function x210226_OnAccept( sceneId, selfId )
	--加入任务到玩家列表
	AddMission( sceneId,selfId, x210226_g_MissionId, x210226_g_ScriptId, 1, 0, 0 )		--添加任务
	misIndex = GetMissionIndexByID(sceneId,selfId,x210226_g_MissionId)			--得到任务的序列号
	...
	Msg2Player(  sceneId, selfId,"#Y接受任务：送馒头",MSG2PLAYER_PARA )
...
function x210226_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	if x210226_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		if ret > 0 then
			...
			Msg2Player(  sceneId, selfId,"#Y完成任务：送馒头",MSG2PLAYER_PARA )
