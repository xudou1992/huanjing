...
x210224_g_MissionTarget="阅读#G大理皇宫门口#W的#R布告牌#W#{_INFOAIM148,40,2,-1}，然后回#G大理城五华坛#W找#R赵天师#W#{_INFOAIM160,157,2,赵天师}。#b#G（请用左键点选带下划线的坐标，帮助您找到该NPC）#l"
...
function x210224_OnAccept( sceneId, selfId )
	...
	Msg2Player(  sceneId, selfId,"#Y接受任务：去看看布告",MSG2PLAYER_PARA )
...
function x210224_OnSubmit( sceneId, selfId, targetId,selectRadioId )
	if x210224_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		if ret > 0 then
			...
			Msg2Player(  sceneId, selfId,"#Y完成任务：去看看布告",MSG2PLAYER_PARA )
