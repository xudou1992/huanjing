...
--任务入口函数
--**********************************
function x210236_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	--发送任务需求的信息
	BeginEvent(sceneId)
		...
		if ret > 0 then
			...
			if	ret > 0 then
				...
				if ret > 0 then
					...
					Msg2Player(  sceneId, selfId,"#Y完成任务：御赐精纺衣",MSG2PLAYER_PARA )
				end
			else
				--删除物品不成功
				BeginEvent(sceneId)
...
--杀死怪物或玩家
--**********************************
function x210236_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
end
