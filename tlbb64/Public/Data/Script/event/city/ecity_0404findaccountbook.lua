...
x600021_g_ItemIdx = x600021_g_StrForePart + 1										-- 任务物品的存储位置
x600021_g_ItemCountIdx = x600021_g_StrForePart + 2									-- 任务物品数量的存储位置
...
--任务入口函数
--**********************************
function x600021_OnDefaultEvent( sceneId, selfId, targetId )	--点击该任务后执行此脚本
	if GetName( sceneId, targetId ) ~= x600021_g_Name then		--判断该npc是否是对应任务的npc
		return
	end
...
function x600021_OnAccept( sceneId, selfId, targetId )
	...
	if GetLevel( sceneId, selfId ) < 40 then
		CallScriptFunction( x600021_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "等级不够" )
		return
	end
	...
	if not validMissions then
		...
		CallScriptFunction( x600021_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "无法找到合适的任务" )
		return
	end
	...
	BeginEvent( sceneId )
		local missionInfo
		if random(2) == 1 then
			missionInfo = format( "    本城的帐房先生路过%s时，被那里的%s袭击了，#Y%s#W丢失真是麻烦，你去帮我找回来吧。",
				strNpcSceneDesc, strNpcName, strDemandItemName )
		else
			missionInfo = format( "    在%s有一群%s经常袭击路人，昨天居然连本城的帐房先生也敢动，这不，#Y%s#W也失落了，只能麻烦你了。",
				strNpcSceneDesc, strNpcName, strDemandItemName )
		end

		AddText( sceneId, missionInfo )
		AddText( sceneId, "    你接受了任务：" .. x600021_g_MissionName )
	EndEvent( sceneId )
...
function x600021_OnKillObject( sceneId, selfId, objdataId, objId )	--参数意思：场景号、玩家objId、怪物表位置号、怪物objId
	...
	for i = 0, allOwnersCount - 1 do
		...
		if IsHaveMission( sceneId, curOwner, x600021_g_MissionId ) > 0 and
		   GetScriptIDByMissionID( sceneId, curOwner, x600021_g_MissionId ) == x600021_g_ScriptId then		--如果这个人拥有任务
