...
function x713624_OnDefaultEvent( sceneId, selfId, targetId, nNum, npcScriptId, bid )
	if bid then
		if 0 == nNum then	--学习
			...
			local studyMsg = format("如果你达到%d级并且肯花费#{_EXCHG%d}和%d点帮贡就可以学会强身技能。你决定学习么？", lv, money, con)
			AddText(sceneId,studyMsg)
			--确定学习按钮
					AddNumText(sceneId,x713624_g_ScriptId,"我确定要学习", 6, 3)
			--取消学习按钮
					AddNumText(sceneId,x713624_g_ScriptId,"我只是来看看", 8, 4)
			EndEvent(sceneId)
...
function x713624_OnEnumerate( sceneId, selfId, targetId, bid, nNum)
	if bid then
		if 1 == nNum then
			...
			if ret > 0 then AddNumText(sceneId,x713624_g_ScriptId,"学习"..x713624_g_AbilityName.."技能", 12, 0) end
			return
		elseif 2 == nNum then
			local ret = CallScriptFunction( CITY_BUILDING_ABILITY_SCRIPT, "OnCityCheck",sceneId, selfId, x713624_g_AbilityID, bid, 6)
			if ret > 0 then AddNumText(sceneId,x713624_g_ScriptId,"升级"..x713624_g_AbilityName.."技能", 12, 1) end
			return
		elseif 3 == nNum then
			AddNumText(sceneId,x713624_g_ScriptId,"我想了解强身",11,2)
			AddNumText(sceneId,x713624_g_ScriptId,"返回",8,5)
		end
	else
		-- 目前强身技能必须在城市里才能学习、升级、了解
	end
