...
--学习界面要说的话
x713515_g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会玄冰术技能。你决定学习么？"
...
function x713515_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId )
	...
	--判断是否已经学会了玄冰术,如果学会了,则提示已经学会了
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"你已经学会玄冰术技能了");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--确定学习按钮
					AddNumText(sceneId,x713515_g_ScriptId,"我确定要学习", 6, 2)
			--取消学习按钮
					AddNumText(sceneId,x713515_g_ScriptId,"我只是来看看", 8, 3)
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
...
function x713515_OnEnumerate( sceneId, selfId, targetId )
		...
		--if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713515_g_ScriptId,"学习玄冰术技能", 12, 0)
		--end
		return
end
