...
--学习界面要说的话
x713505_g_MessageStudy = "如果你达到%d级并且肯花费#{_EXCHG%d}就可以学会铸造技能。你决定学习么？"

--技能编号
x713505_g_AbilityID = ABILITY_ZHUZAO

--技能名称
x713505_g_AbilityName = "铸造"

x713505_g_Name1 = "耶律大石"
--**********************************
--任务入口函数
--**********************************
function x713505_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid )
	...
	--判断是否已经学会了铸造,如果学会了,则提示已经学会了
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"你已经学会"..x713505_g_AbilityName.."技能了");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--确定学习按钮
					AddNumText(sceneId,x713505_g_ScriptId,"我确定要学习", 6, 2)
			--取消学习按钮
					AddNumText(sceneId,x713505_g_ScriptId,"我只是来看看", 8, 3)
			EndEvent(sceneId)
	...
	elseif ButtomNum == 2 then			--如果点击的是“我确定要学习”
		...
		if ret and ret == 1 then
			...
			--在npc聊天窗口通知玩家已经学会了
			BeginEvent(sceneId)
				AddText(sceneId,"你学会了"..x713505_g_AbilityName.."技能")
			EndEvent( )
...
function x713505_OnEnumerate( sceneId, selfId, targetId, bid )
		if bid then
			...
			if ret > 0 then AddNumText(sceneId,x713505_g_ScriptId,"学习"..x713505_g_AbilityName.."技能", 12, 0) end
			return
		end
		--如果不到等级则不显示选项
		...
		if ret and ret == 1 then
			AddNumText(sceneId,x713505_g_ScriptId,"学习"..x713505_g_AbilityName.."技能", 12, 0)
		end
		return
end
...
function x713505_StudyInCity(sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid)
	if bid then
		if 0 == ButtomNum then
			...
			local studyMsg = format("如果你达到%d级并且肯花费#{_EXCHG%d}和%d点帮贡就可以学会"..x713505_g_AbilityName.."技能。你决定学习么？", lv, money, con)
