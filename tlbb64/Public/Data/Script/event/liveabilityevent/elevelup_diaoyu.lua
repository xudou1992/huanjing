...
function x713569_OnDefaultEvent( sceneId, selfId, targetId, nNum, npcScriptId, bid )
	...
	if AbilityLevel < 1	then
		BeginEvent(sceneId)
			strText = "你还没有学会"..x713569_g_AbilityName.."技能！"
	...
	if AbilityLevel >= x713569_g_MaxLevel then
		BeginEvent(sceneId)
			strText = "我只能教你1-5级的"..x713569_g_AbilityName.."技能,请到帮派中学习更高级的"..x713569_g_AbilityName.."."
			AddText(sceneId,strText)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
...
function x713569_OnEnumerate( sceneId, selfId, targetId, bid )
		if bid then
			...
			if ret > 0 then AddNumText(sceneId,x713569_g_ScriptId,"升级"..x713569_g_AbilityName.."技能", 12, 1) end
			return
		end
		--如果不到等级则不显示选项
		--old
		--if GetLevel(sceneId,selfId) >= LEVELUP_ABILITY_DIAOYU[1].HumanLevelLimit then
		--	AddNumText(sceneId,x713569_g_ScriptId,"升级"..x713569_g_AbilityName.."技能", 12, 1)
		--end
		--new
