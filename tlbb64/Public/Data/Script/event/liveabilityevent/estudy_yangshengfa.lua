...
--学习界面要说的话
x713528_g_MessageStudy = "只要你肯花费#{_EXCHG%d}就可以学会养生法技能。你决定学习么？"

x713528_g_AbilityName = "养生法"
...
function x713528_OnDefaultEvent( sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid )
	...
	--判断是否已经学会了养生法,如果学会了,则提示已经学会了
	if AbilityLevel >= 1 then
		BeginEvent(sceneId)
        	AddText(sceneId,"你已经学会养生法技能了");
        	EndEvent(sceneId)
	...
	if ButtomNum == 0 then
		...
		if ret and ret == 1 then
			...
			--确定学习按钮
					AddNumText(sceneId,x713528_g_ScriptId,"我确定要学习", 6, 2)
			--取消学习按钮
					AddNumText(sceneId,x713528_g_ScriptId,"我只是来看看", 8, 3)
			EndEvent(sceneId)
...
function x713528_OnEnumerate( sceneId, selfId, targetId, bid )
		if bid then
			...
			if ret > 0 then AddNumText(sceneId,x713528_g_ScriptId,"学习"..x713528_g_AbilityName.."技能", 12, 0) end
			return
		end
		--如果不到等级则不显示选项
		--if GetLevel(sceneId,selfId) >= 10 then
			AddNumText(sceneId,x713528_g_ScriptId,"学习养生法技能", 12, 0)
		--end
		return
end
...
function x713528_StudyInCity(sceneId, selfId, targetId, ButtomNum,g_Npc_ScriptId,bid)
	if bid then
		if 0 == ButtomNum then
			...
			local studyMsg = format("如果你达到%d级并且肯花费#{_EXCHG%d}和%d点帮贡就可以学会"..x713528_g_AbilityName.."技能。你决定学习么？", lv, money, con)
