...
--任务是否完成
x808082_g_Mission_IsComplete = 0		--任务参数的第0位
--龟兔赛跑大理打卡标记
x808082_g_RecordIdx 				 = 3		--任务参数的第3位 1.洛阳 2.苏州 3.大理
--孔明灯完成数量标记
x808082_g_KongMing_Lighten	 = 4		--任务参数的第4位
--任务脚本号记录
x808082_g_MissScriptID_Idx	 = 5		--任务参数的第5位
--任务发布NPC标记
x808082_g_AcceptNPC_Idx			 = 6		--任务参数的第6位 1.洛阳NPC 2.苏州NPC 3.大理NPC
--龟兔赛跑任务每天轮数标记
x808082_g_RoundIndex 				 = 7		--任务参数的第7位


x808082_g_AcceptMission_IDX		= 720	--龟兔赛跑接收任务索引
x808082_g_CompleteMission_IDX	= 721	--龟兔赛跑提交任务索引
x808082_g_MissionInfo_IDX			= 722	--任务说明索引

x808082_g_Jion_Count = 0						--报名人数标记
x808082_g_Join_CountMax = 50				--报名人数上限

x808082_g_KongMingLighten_MaxCnt	=	24	-- 龟兔赛跑活动需要点燃的孔明灯总数

--所拥有的事件ID列表
x808082_g_EventList	= {}

x808082_g_Impact_Accept_Mission 	= 47	-- 接受任务时的特效ID
x808082_g_Impact_Light_Complete 	= 38	-- 点亮孔明灯时的特效
x808082_g_Impact_Transport_Mission = 113 -- 运输状态特效
x808082_g_Impact_GodOfFire_Mission = 5929 -- 龟兔赛跑状态特效
x808082_g_PlayerSlow_LVL					= 20	-- 接受任务的最低等级
...
x808082_g_ItemId = { id=40004444,num=1 }				-- 任务物品
...
function x808082_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x808082_g_Name then
		x808082_NotifyTip( sceneId, selfId, "接受任务失败" )					
		return 0
	end
...
function x808082_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x808082_g_ScriptId, "我要参加龟兔赛跑", 7, x808082_g_AcceptMission_IDX )
	AddNumText( sceneId, x808082_g_ScriptId, "我已经完成了龟兔赛跑", 7, x808082_g_CompleteMission_IDX )
	AddNumText( sceneId, x808082_g_ScriptId, "龟兔赛跑活动介绍", 11, x808082_g_MissionInfo_IDX )
...
function x808082_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808082_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808082_g_ItemId.id) >= x808082_g_ItemId.num then
	  	...
	  	x808082_NotifyTip( sceneId, selfId, "您的物品现在不可用或已被锁定。" )			
			return 0
  	end
  end
...
function x808082_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	--判断该npc是否是对应任务的npc
	if LuaFnGetName( sceneId, targetId ) ~= x808082_g_Name then
		x808082_NotifyTip( sceneId, selfId, "提交任务失败" )					
		return 0
	end
...
--杀死怪物或玩家
--**********************************
function x808082_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
end
