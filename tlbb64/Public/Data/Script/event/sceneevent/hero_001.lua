...
--x212128_g_MissionContinue="  生命如果能够重来一次，我会怎么选择呢？既然真实和虚幻无从分辨，为何不听从心灵的召唤做出抉择呢。"
...
function x212128_OnAccept( sceneId, selfId, targetId )
	...
	local strText = format("#Y接受任务：英雄不怕远征难。")
	Msg2Player(sceneId, selfId, strText,MSG2PLAYER_PARA)
 	BeginEvent(sceneId)
		AddText(sceneId, strText)
  EndEvent(sceneId)
...
function x212128_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x212128_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player( sceneId, selfId,"#Y完成任务：英雄不怕远征难",MSG2PLAYER_PARA )
	end
end
